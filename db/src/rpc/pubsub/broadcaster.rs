...
pub struct Broadcaster<T> {
    ...
    subscriptions: DashMap<u32, (i64, mpsc::Sender<T>)>,
}

impl<T> Default for Broadcaster<T> {
    fn default() -> Self {
        Self {
            topics: DashMap::with_capacity(32),
            subscriptions: DashMap::with_capacity(32),
        }
    }
}
...
where
    T: fmt::Debug + Send + Sync + 'static + Clone,
{
    fn subscribe(self, name: impl Into<String>) -> (u32, mpsc::Receiver<T>) {
        let id = {
        ...
        self.subscriptions.insert(id, (Utc::now().timestamp(), tx));
    ...
    fn publish(self, name: impl Into<String>, value: T) {
        let name = name.into();
        tokio::spawn(async move {
            ...
            if let Some(topic) = self.topics.get(&name) {
                ...
                for id in subscriptions.into_iter() {
                    if let Some(mut tx) = self.subscriptions.get_mut(&id) {
                        tx.0 = Utc::now().timestamp();
                        if let Err(e) = tx.1.send(value.clone()).await {
...
mod tests {
    ...
    async fn publish_should_work() {
        ...
        broadcaster.publish(topic, "hello world".into());
    ...
    async fn subscribe_abnormal_quit_should_be_removed_on_next_publish() {
        ...
        broadcaster
            .clone()
            .publish(topic, "hello world".into());
